            ...
            <form className="form-post" onSubmit={handleSubmit(onSubmit)} >
                ...
                <div className="cores">
                    ...
                    {nomeInput !== "" && (
                        <div className="previatotal">
                            <h1>Veja como esta ficando:</h1>
                            <article className="import { Header } from "../../../components/header"
import { FormEvent, useState, useEffect, useContext } from "react"
import { BsFillTrash3Fill } from 'react-icons/bs'
import { PiBroomBold } from "react-icons/pi"
import { } from "../../../assets/css/index.css"
...
import {
    addDoc, collection, onSnapshot,
    query, orderBy, doc, deleteDoc, where, getDocs, updateDoc, getFirestore,
} from "firebase/firestore"
import { db } from "../../../services/firebaseConnection"
import { z } from "zod"
import { useForm } from "react-hook-form"
...
export function Admin() {
    ...
    const deleteItem = async (id: string) => {
        ...
        await deleteDoc(doc(db, "Post-it", id));
    };
    useEffect(() => {
        const anotsRef = collection(db, "Post-it");
        const queryRef = query(anotsRef, orderBy("created", "asc"));

        const unsub = onSnapshot(queryRef, (snapshot) => {
            let anotacoes = [] as AnotProps[];
    ...
    const updateItem = async (id: string, novoNome: string, novaDescricao: string, novaColor: string, novoBg: string) => {
        const db = getFirestore();
        await updateDoc(doc(db, "Post-it", id), { nome: novoNome, descricao: novaDescricao, color: novaColor, bg: novoBg });
        setEditId(null);
    };
    const editItem = (anotprops: anotprops) => {
        setNomeInput(anotprops.nome);
        setDescInput(anotprops.descricao);

        setColorBackgroundInput(anotprops.bg)
        setColorTextInput(anotprops.color)

    };

    const addItem = async (novoNome: string, novaDescricao: string, novaColor: string, novoBg: string) => {
        const db = getFirestore();
        await addDoc(collection(db, "Post-it"), { nome: novoNome, descricao: novaDescricao, color: novaColor, bg: novoBg });
    };
    ...
    return (
        <div >
            ...
            <form onSubmit={(e) => {
                e.preventDefault();
                if (editId) {
                    updateItem(editId, nomeInput, descInput, colorBackgroundInput, colorTextInput);
                } else {
                    addItem(nomeInput, descInput, colorBackgroundInput, colorTextInput);
                }
                setNomeInput("");
                setDescInput("");
                setColorBackgroundInput("#000000")
                ...
                <div className="cores">
                    ...
                    <div className="div-btn-cadastrar">
                        ...
                        <button className="btn-cadastrar" type="submit">{editId ? "Atualizar" : "Adicionar"}</button>
                    </div>
            ...
            <div className="map-postit">
                ...
                {anots.map((anot) => (
                    <article id="art" className="previa" key={anot.id}
                        ...
                        <p>{anot.nome}</p>
                        <div>
                            <button className="icon-ger" onClick={() => editItem(anot)} >
                                <BsPencil size={30} color="#FAC935" />
                            </button>
                            <button className="icon-ger" onClick={() => deleteItem(anot.id)} >
